...
def landing(request):
    ...
    # Only serve landing page for landing subdomain
    if hasattr(request, 'subdomain_type') and request.subdomain_type not in ['landing', 'public']:
        from django.http import Http404
        raise Http404("Not found")

    # For authenticated users on landing page, redirect to app
    if request.user.is_authenticated and request.subdomain_type == 'landing':
        return redirect('https://app.trainerhubb.app/dashboard')
